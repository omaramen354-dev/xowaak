 ...
 *
 * Shader (vert/frag) is kept verbatim. Changes made for this codebase:
 *  - typed props
 *  - upstream ends in `return ;` (renders nothing); restored to a real div
 *  - DPR clamped to 1.5 and the loop skipped while the tab is hidden
 *  - live props read through a ref so changing them never rebuilds the GL
 *    context (upstream re-ran the whole effect on every prop change)
 *  - honours prefers-reduced-motion by freezing time
 */
...
export function Orb({
  ...
  // Live values for the render loop. Assigning in an effect (not during
  // render) keeps React's compiler happy and is safe under concurrent mode.
  ...
  useEffect(() => {
    ...
    function resize() {
      if (!container) return;
      // Clamped: this is a soft glow, full DPR on a 3x screen buys nothing.
    ...
    let lastBg = propsRef.current.backgroundColor;
    ...
    const update = (t: number) => {
      ...
      // Only rebuild the Vec3 when the colour actually changes.
      ...
      if (p.rotateOnHover && effectiveHover > 0.5 && !reduced) {
